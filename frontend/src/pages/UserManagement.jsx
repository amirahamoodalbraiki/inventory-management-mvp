...
export default function UserManagement() {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white">
      <div className="flex-1 px-4 sm:px-6 py-8 max-w-full md:max-w-[1000px] w-full mx-auto">
        {/* header */}
        <header className="flex items-center justify-between gap-3">
          <h1 className="m-0 text-[#253A82] text-2xl sm:text-[28px] font-extrabold">
            Users
          </h1>
          <button
            onClick={handleAdd}
            className="rounded-lg bg-[#253A82] text-white border border-[#88A2FF] text-sm sm:text-[13px] font-semibold cursor-pointer px-3 sm:px-4 py-2"
          >
            Add User
          </button>
        </header>
...
<div className="rounded-lg bg-white pl-3 pr-3 py-2.5 w-full flex items-center gap-2 border border-[#88A2FF]">
  <svg
    ...
    height="16"
    viewBox="0 0 24 24"
    className="text-[#253A82] shrink-0"
  >
    <circle
      cx="11"
      cy="11"
  ...
  />
          ...
          <label className="relative inline-block">
            <select
              ...
              className="w-[120px] py-[10px] pl-[14px] pr-9 rounded-[8px] bg-white border border-[#88A2FF] text-[#253A82] text-[14px] font-semibold cursor-pointer appearance-none"
            >
              ...
              <table className="w-full min-w-[820px] border-collapse">
                <thead>
                  <tr className="bg-[#88A2FF]/20">
                    <th className="text-left px-4 py-3 border-b border-[#88A2FF] font-semibold text-xs sm:text-[13px] text-[#253A82]">
                      Name
                    </th>
                    <th className="text-left px-4 py-3 border-b border-[#88A2FF] font-semibold text-xs sm:text-[13px] text-[#253A82]">
                      Email
                ...
                <tbody>
                    ...
                    return (
                      <tr key={u.id} className="border-b border-[#88A2FF]">
                        <td className="px-4 py-[12px] sm:py-[14px] text-sm sm:text-[14px] text-[#253A82]">
                          <span className="block max-w-[220px] sm:max-w-none truncate">
                            {u.name || "—"}
                          </span>
                        </td>
                        <td className="px-4 py-[12px] sm:py-[14px] text-sm sm:text-[14px] text-[#253A82]">
                          <span className="block max-w-[260px] sm:max-w-none truncate">
                            {u.email}
                          </span>
                        </td>
                        <td className="px-4 py-[12px] sm:py-[14px] text-sm sm:text-[14px] text-[#253A82]">
                          <span className="block max-w-[180px] sm:max-w-none truncate">
                            {u.phone || "—"}
                          </span>
                        </td>
                        <td className="px-4 py-[12px] sm:py-[14px] text-sm">
                          ...
                          <div className="flex gap-2">
                            ...
                            <button
                              ...
                              className={`bg-transparent border-0 p-0 font-bold ${
                                isMe
                                  ? "text-[#253A82]/40 cursor-not-allowed"
                                  : "text-[#253A82] cursor-pointer"
                              }`}
                              title={
                                isMe
                                  ? "You can’t delete your own account"
                  ...
                  {paged.length === 0 && (
                    <tr>
                      <td
                        colSpan={5}
                        className="px-4 py-[14px] text-center text-[#253A82]"
                      >
                        No users found
                      </td>
                    </tr>
            ...
            <div className="flex items-center justify-center gap-3 sm:gap-4 p-2.5 mt-3">
              ...
              {getDisplayPages(page, pageCount).map((p, idx) =>
                p === "…" ? (
                  <span
                    key={`gap-${idx}`}
                    className="text-xl sm:text-[24px] text-[#253A82]"
                  >
                    …
                  </span>
                ) : (
                  <button
                    key={p}
                    onClick={() => setPage(p)}
                    className={`rounded-full border-0 text-sm sm:text-[15px] font-medium cursor-pointer w-9 h-9 sm:w-12 sm:h-12 ${
                      p === page
                        ? "bg-[#253A82] text-white"
                        : "bg-transparent text-[#253A82]"
                    }`}
                  >
                    {p}
                  </button>
